...
function OrionLib:MakeWindow(WindowConfig)
	-- Config
		...
		local function LoadBackground(name)
			...
			warn("[BetterOrion] Failed to load background: " .. tostring(name))
			return nil
		end
	...
	-- Elements
		...
		if WindowConfig.Background then
			local bgAsset = LoadBackground(WindowConfig.Background)
			if bgAsset then
	...
	-- Local set functions
		...
		function TabFunction:SetBackground(name, transparency)
			local bgAsset = LoadBackground(name)
			if not bgAsset then return end
			if BackgroundImage then
				BackgroundImage.Image = bgAsset
				BackgroundImage.ImageTransparency = transparency or 0.15
			else
				BackgroundImage = Create("ImageLabel", {
					Image = bgAsset,
					Size = UDim2.new(1, 0, 1, 0),
		...
		function TabFunction:RemoveBackground()
			if BackgroundImage then
